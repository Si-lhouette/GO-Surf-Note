...
def load_intrin_K(filepath, H, W):
    file = open(filepath, "r")
...
class RealsenseDataset(torch.utils.data.Dataset):
    def __init__(self, basedir, load=True, trainskip=1, downsample_factor=1, crop=0, near: float = 0.01,
        ...
        #                   [0., focal, (self.H - 1) / 2],
        #                   [0., 0., 1.]])
        self.K = load_intrin_K(os.path.join(self.basedir, 'focal.txt'), self.H, self.W)
        print("K: ", self.K)
    ...
    def get_all_frames(self):
        for i, frame_id in enumerate(self.frame_ids):
            ...
            if self.downsample_factor > 1:
                raise ValueError("Only support downsample_factor = 1")
            ...
            K = torch.tensor(self.K, dtype=torch.float32).to(self.device)
